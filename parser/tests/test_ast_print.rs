...
fn test_print_initializer_expression_ast2() {
    let stmt = parser::parse_program("contract Test { uint i; }").unwrap();
...
fn test_print_program_ast1() {
    let stmt = parser::parse_program("contract Test { uint i;\nuint a = 3; }").unwrap();
    let ast = print::program_to_str(&stmt);
    assert_eq!(
        ast.print_ast(),
